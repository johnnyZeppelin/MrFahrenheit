...
def ask_model(client, model_name, image_path, task="brand"):
    ...
    # added
    img_pth = Path(img_key)
    if not img_pth.exists():
        return f"ERROR: {img_pth} does not exist."
    ...
    img_data_url = f"data:image/{img_pth.suffix[1:]};base64,{img_b64}"
...
if __name__=="__main__":
    ...
    for model_name in vlm_models:
        ...
        for transform_path in dataset_dir.iterdir():
            ...
            # Add start below
            for dataset_path in transform_path.iterdir():
                    ...
                    for brand_path in category_path.iterdir():
                        ...
                        for img_file in tqdm(img_files, desc=f"{transformation}/{category}/{brand}", leave=False, ncols=100):
                            ...
                            results.append({
                                ...
                                "dataset": dataset_name,   # new field
                                "category": category,
                                "brand": brand,
                                "image": img_file.name,
...
'''
Next, it can generate LaTeX-ready tables automatically from the results.
It adds two tables:
1. Overall Accuracy + LTC per Model
2. Per-Category Accuracy per Model (optional, more detailed)
'''
def generate_latex_table_overall(all_results, vlm_models, out_file="vlm_overall_table.tex"):
    """
    Generate LaTeX table: Overall Accuracy and Logo-Text Consistency (LTC) per model.
    """
    ...
    for model in vlm_models:
        ...
        consistent = sum(1 for r in model_results if r["logo_text_consistency"])
        acc = correct / total if total > 0 else 0
        ltc = consistent / total if total > 0 else 0
        metrics[model] = {"accuracy": acc, "ltc": ltc}
    ...
    with open(out_file, "w", encoding="utf-8") as f:
        ...
        f.write("\\hline\n")
        f.write("Model & Accuracy & Logo-Text Consistency \\\\\n")
        f.write("\\hline\n")
        for model, vals in metrics.items():
            f.write(f"{model} & {vals['accuracy']:.2%} & {vals['ltc']:.2%} \\\\\n")
        f.write("\\hline\n")
        f.write("\\end{tabular}\n")
        f.write("\\caption{Overall Accuracy and Logo-Text Consistency (LTC) of VLMs}\n")
        f.write("\\label{tab:vlm_overall}\n")
        f.write("\\end{table}\n")

    print(f"[INFO] LaTeX table saved to {out_file}")


def generate_latex_table_per_category(metrics_per_category_all_models, out_file="vlm_per_category_table.tex"):
    """
    Generate LaTeX table: Accuracy per category per model.
    """
    categories = list(next(iter(metrics_per_category_all_models.values())).keys())
    models = list(metrics_per_category_all_models.keys())
    ...
    with open(out_file, "w", encoding="utf-8") as f:
        ...
        f.write("\\hline\n")
        for model in models:
            vals = [f"{metrics_per_category_all_models[model][cat]['accuracy']:.2%}" for cat in categories]
            f.write(f"{model} & " + " & ".join(vals) + " \\\\\n")
        f.write("\\hline\n")
        f.write("\\end{tabular}\n")
        f.write("\\caption{Per-category accuracy of VLMs}\n")
        f.write("\\label{tab:vlm_per_category}\n")
        f.write("\\end{table}\n")
...
# -------------------
# Call after experiment
# -------------------
generate_latex_table_overall(all_results, vlm_models, out_file=FIGURES_DIR / "vlm_overall_table.tex")
generate_latex_table_per_category(metrics_per_category_all_models, out_file=FIGURES_DIR / "vlm_per_category_table.tex")


'''
Next can realize automate figures + LaTeX tables generation for each model.
'''
# -------------------
# Generate per-model LaTeX tables alongside figures
# -------------------
for model_name in vlm_models:
    sanitized_name = model_name.replace("/", "_")
    model_fig_dir = FIGURES_DIR / sanitized_name

    # --- Overall table ---
    overall_table_file = model_fig_dir / f"{sanitized_name}_overall_table.tex"
    generate_latex_table_overall(
        [r for r in all_results if r["model"] == model_name],
        [model_name],
        out_file=overall_table_file
    )

    # --- Per-category table ---
    per_cat_table_file = model_fig_dir / f"{sanitized_name}_per_category_table.tex"
    generate_latex_table_per_category(
        {model_name: metrics_per_category_all_models[model_name]},
        out_file=per_cat_table_file
    )

    # --- Multi-model comparison tables ---
multi_model_overall_table = FIGURES_DIR / "multi_model_overall_table.tex"
generate_latex_table_overall(all_results, vlm_models, out_file=multi_model_overall_table)

multi_model_per_category_table = FIGURES_DIR / "multi_model_per_category_table.tex"
generate_latex_table_per_category(metrics_per_category_all_models, out_file=multi_model_per_category_table)

print("[INFO] All LaTeX tables generated alongside PDF figures.")
